  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-brand-blue">Courses</h1>
          <p className="mt-1 text-sm text-brand-muted">Manage the e-learning course catalog.</p>
        </div>
        <Link
        ...
        {courses.length === 0 ? (
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {courses.map((course) => (
                <tr key={course.slug}>
                  <td className="px-6 py-4 font-medium text-brand-blue">{course.title}</td>
                  <td className="px-6 py-4 text-brand-muted">{course.category}</td>
                  <td className="px-6 py-4 text-brand-muted">
                    {course.price === 0 ? "Free" : `₦${course.price.toLocaleString("en-NG")}`}
                  </td>
                  <td className="px-6 py-4">
                    <StatusBadge status={course.status === "archived" ? "draft" : course.status} />
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex justify-end gap-1">
                      ...
                      <DeleteButton
                        deleteUrl={`/api/admin/courses/${course.slug}`}
